...
let lastCell = null;
export function pieceMoveHandler(e, startingCoords, currPiece, board) {
  if (e.buttons === 1) {
...
export function endMoveHandler(
  ...
  if (validationResult[2]) {
    setPromotion({ toPlaceCell: cellInBoard, startCell: cell, figure });
  } else if (validationResult[0]) {
    const castlingCheck = figure.makeMove(
      cellInBoard,
      cell,
      validationResult[1]
    );
...
function endMoveWithoutDragging(
  e,
  figure,
  setBoard,
  startingCell,
  ...
  if (validation[2]) {
    setPromotion({ toPlaceCell: toPlaceCell, startCell: startingCell,figure });
